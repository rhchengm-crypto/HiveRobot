#!/usr/bin/env python3
"""Web control panel for left_arm_v2_6 with HP60C RGB-D stream.

Run on the Orin where ROS HP60C topics and the left arm serial port exist.
The page exposes only the tested v2.6 actions plus a debug command/output
...
DEFAULT_ARM_SCRIPT = os.path.join(SCRIPT_DIR, "left_arm_v2_6.py")
DEFAULT_MOVE_SCRIPT = os.path.join(SCRIPT_DIR, "left_arm_v2_6_move_library.py")
DEFAULT_RUN_LOG = os.path.join(SCRIPT_DIR, "data", "left_arm_v2_6_web_runs.jsonl")
DEFAULT_MOVES_FILE = os.path.join(SCRIPT_DIR, "data", "left_arm_v2_6_saved_moves.json")
...
<body>
  <header>
    <h1>HiveRobot Left Arm Control <span style="color:#7f8c98;font-size:13px;">v2.6</span></h1>
    <nav>
  ...
  <main>
    ...
    <aside>
      ...
      <section class="panel">
        <h2>Joint Nudge <span style="color:#7f8c98;font-size:12px;">v2.6</span></h2>
      ...
      <section class="panel">
        <h2>Saved Moves <span style="color:#7f8c98;font-size:12px;">web controller 1.2</span></h2>
        <div class="move-form">
          <button type="button" onclick="runAction('low-torque')">Low Torque</button>
          <div class="move-row">
            <input id="moveName" type="text" placeholder="Move name">
            <button type="button" onclick="captureMove()">Capture New Move</button>
          </div>
          <div class="move-row">
            <select id="moveSelect"></select>
            <button type="button" class="primary" onclick="replayMove()">Replay</button>
          </div>
        </div>
      </section>
      <section class="panel">
  ...
  <script>
    ...
    async function loadMoves(selectedName) {
      try {
        const res = await fetch('/api/moves?t=' + Date.now(), { cache: 'no-store' });
        const data = await res.json();
        ...
        setStatus('Move list refresh failed:\\n' + err);
      }
    }

    async function waitForRunIdle(actionPrefix, timeoutMs) {
      const deadline = Date.now() + timeoutMs;
      while (Date.now() < deadline) {
        const res = await fetch('/api/state?t=' + Date.now(), { cache: 'no-store' });
        const data = await res.json();
        const run = data.run || null;
        if (run && String(run.action || '').startsWith(actionPrefix) && !run.running) return run;
    ...
    async function captureMove() {
      const name = document.getElementById('moveName').value.trim();
      if (!name) {
        setStatus('Move name is required.');
        return;
      }
      if (!confirm('Capture new move: ' + name + '?')) return;
      try {
        const res = await fetch('/api/move/capture', {
          method: 'POST',
          cache: 'no-store',
          headers: { 'Content-Type': 'application/json' },
        ...
        document.getElementById('command').textContent = data.command_text || JSON.stringify(data.command || [], null, 2);
        document.getElementById('output').textContent = formatOutput(data);
        setStatus(data.ok ? 'Capture move running/finished: ' + name : 'Capture move failed: ' + name);
        if (data.ok) {
          const finished = await waitForRunIdle('capture-move:' + name, 8000);
          if (finished) {
        ...
        setStatus('Capture move request failed:\\n' + err);
      }
    ...
    async function replayMove() {
      ...
      if (!confirm('Replay move via table clearance first: ' + name + '?')) return;
      try {
...
def move_cmd_prefix(cfg: ControlConfig) -> list:
    cmd = []
    if cfg.use_sudo:
        cmd.extend(["sudo", "-n"])
    cmd.extend([cfg.python_bin, "-u", cfg.move_script])
    return cmd
...
def build_arm_command(cfg: ControlConfig, action: str) -> list:
    ...
    if action == "low-torque":
        return move_cmd_prefix(cfg) + ["low-torque"]
    raise ValueError("unknown action: " + action)


def build_capture_move_command(cfg: ControlConfig, name: str) -> list:
    return move_cmd_prefix(cfg) + [
        "capture-move",
        "--name",
        name,
        "--note",
...
def build_replay_move_command(cfg: ControlConfig, name: str) -> list:
    return move_cmd_prefix(cfg) + [
        "replay-move",
        "--name",
        name,
...
def build_list_moves_command(cfg: ControlConfig) -> list:
    return move_cmd_prefix(cfg) + ["list-moves", "--moves-file", cfg.moves_file]
...
def build_emergency_stop_command(cfg: ControlConfig) -> list:
    ...
    code = (
        ...
        "from left_arm_v2_6 import LeftArmV2, DEFAULT_JOINTS\n"
        "arm = LeftArmV2()\n"
        "try:\n"
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            if path == "/api/nudge":
                self.start_nudge()
                return
            if path == "/api/move/capture":
                self.start_capture_move()
                return
            if path == "/api/move/replay":
                self.start_replay_move()
                return
            self.send_error(HTTPStatus.NOT_FOUND)
        ...
        def send_moves(self) -> None:
            cmd = build_list_moves_command(cfg)
            try:
                completed = subprocess.run(cmd, check=False, text=True, capture_output=True, timeout=4.0)
                if completed.returncode != 0:
                    ...
                    return
                self.send_json(json.loads(completed.stdout or '{"moves": []}'))
            except Exception as exc:
                self.send_json({"moves": [], "error": str(exc), "command_text": " ".join(cmd)}, HTTPStatus.OK)
        ...
        def start_capture_move(self) -> None:
            try:
                body = self.read_json_body()
                name = str(body.get("name", "")).strip()
            except Exception as exc:
                self.send_json({"ok": False, "error": f"invalid capture payload: {exc}"}, HTTPStatus.BAD_REQUEST)
                return
            if not name:
                self.send_json({"ok": False, "error": "move name is required"}, HTTPStatus.BAD_REQUEST)
                return
            cmd = build_capture_move_command(cfg, name)
            if not cfg.execute_enabled:
                self.send_json(
                    {
                        "ok": False,
                ...
                return
            cancelled = run_state.cancel_current(timeout=1.0, reason="capture_move", expected_actions={"low-torque"})
            if cancelled is not None:
                time.sleep(0.2)
            payload = run_state.start(f"capture-move:{name}", cmd, popen_kwargs={})
            if cancelled is not None:
                payload["cancelled_previous"] = cancelled
            status = HTTPStatus.OK if payload.get("ok") else HTTPStatus.CONFLICT
            self.send_json(payload, status)

        def start_replay_move(self) -> None:
            try:
                body = self.read_json_body()
                name = str(body.get("name", "")).strip()
            except Exception as exc:
                self.send_json({"ok": False, "error": f"invalid replay payload: {exc}"}, HTTPStatus.BAD_REQUEST)
                return
            if not name:
                self.send_json({"ok": False, "error": "move name is required"}, HTTPStatus.BAD_REQUEST)
                return
            cmd = build_replay_move_command(cfg, name)
            if not cfg.execute_enabled:
                self.send_json(
                    {
                        "ok": False,
                ...
                return
            cancelled = run_state.cancel_current(timeout=1.0, reason="replay_move", expected_actions={"low-torque"})
            if cancelled is not None:
                time.sleep(0.2)
        ...
        def start_nudge(self) -> None:
            ...
            if not cfg.execute_enabled:
                ...
                return
            cancelled = run_state.cancel_current(timeout=1.0, reason="nudge", expected_actions={"low-torque"})
            if cancelled is not None:
                time.sleep(0.2)
...
def main() -> None:
    ...
    parser.add_argument("--moves-file", default=DEFAULT_MOVES_FILE)
    ...
    print(f"Left arm v2.6 web control listening on http://{args.host}:{args.port}/")
