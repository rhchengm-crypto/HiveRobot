#!/usr/bin/env python3
"""Clean left-arm bring-up controller v2.6.
...
v2.6 keeps the stable v2.6 clearance/home motion layout, then centralizes
low-shoulder hold/active torque rules so new poses need less manual tuning.
"""
...
LOW_SHOULDER_POSTURE = "low_shoulder"
HOLD_TAU_RULES = {
    "base": {
        "shoulder_front": 3.8,
        "elbow": 2.2,
...
HOLD_TAU_LIMITS = {
    "shoulder_front": (0.8, 3.8),
    "shoulder_side": (0.0, 1.0),
    "shoulder_rotate": (-1.0, 0.0),
    "elbow": (1.0, 5.4),
    "arm_roll": (-0.8, -0.2),
    "wrist_side": (-0.45, 0.0),
    "wrist": (0.35, 0.7),
}
ADAPTIVE_HOLD_TAU_ERROR_DEADBAND_DEG = 1.5
ADAPTIVE_HOLD_TAU_IMPROVEMENT_DEADBAND_DEG = 0.15
ADAPTIVE_HOLD_TAU_STEP_PER_DEG = 0.01
...
ADAPTIVE_TARGET_BIAS_LIMIT_DEG = 5.0
ADAPTIVE_TARGET_BIAS_MIN_HOLD_TAU_SAMPLES = 3
...
NUDGE_LOW_SHOULDER_FRONT_HOLD_TAU = HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]["*"]["shoulder_front"]
NUDGE_LOW_SHOULDER_WRIST_SIDE_HOLD_TAU = HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]["wrist"]["wrist_side"]
...
NUDGE_LOW_SHOULDER_WRIST_HOLD_TAU = {
    "arm_roll": HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]["wrist"]["arm_roll"],
    "shoulder_rotate": HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]["wrist"]["shoulder_rotate"],
    "shoulder_side": HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]["wrist"]["shoulder_side"],
}
NUDGE_LOW_SHOULDER_ARM_ROLL_ACTIVE_TAU = ACTIVE_TAU_RULES[LOW_SHOULDER_POSTURE]["arm_roll"]["negative"]
NUDGE_LOW_SHOULDER_ARM_ROLL_HOLD_TAU = {
    "elbow": HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]["arm_roll"]["elbow"],
    "shoulder_rotate": HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]["arm_roll"]["shoulder_rotate"],
    "wrist_side": HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]["arm_roll"]["wrist_side"],
}
...
NUDGE_LOW_SHOULDER_COMPLIANT_HOLD_GAINS_BY_ACTIVE = {
    active: {
        **POSTURE_HOLD_GAIN_RULES[LOW_SHOULDER_POSTURE].get("*", {}),
        **overrides,
    }
    for active, overrides in POSTURE_HOLD_GAIN_RULES[LOW_SHOULDER_POSTURE].items()
    if active != "*"
}

NUDGE_LOW_SHOULDER_COMPLIANT_HOLD_GAINS_BY_ACTIVE.setdefault(
    "shoulder_front",
    {
        "arm_roll": {"kp": 12.0, "kd": 7.0},
        "shoulder_rotate": {"kp": 12.0, "kd": 5.0},
...
class LeftArmV2:
    ...
    def restart_motors(
        ...
        if hold_seconds > 0:
            ...
            print("v2.6 restart hold current positions seconds=", hold_seconds, "kp=", hold_kp, "kd=", hold_kd)
            self.hold_positions(targets, seconds=hold_seconds, kp=hold_kp, kd=hold_kd)
        print("v2.6 restart after:")
        self.print_status(names)
...
def clamp_tau(name: str, tau: float, limits: Dict[str, Tuple[float, float]]) -> float:
    if name not in limits:
        return tau
    low, high = limits[name]
    return max(low, min(high, tau))


def load_adaptive_hold_tau(path: str = ADAPTIVE_HOLD_TAU_PATH) -> dict:
    if not os.path.exists(path):
        return {"type": "left_arm_v2_6_adaptive_hold_tau", "rules": {}}
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if "rules" not in data or not isinstance(data["rules"], dict):
        raise RuntimeError(f"invalid adaptive hold tau file: {path}")
    return data


def save_adaptive_hold_tau(data: dict, path: str = ADAPTIVE_HOLD_TAU_PATH) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
    data["type"] = "left_arm_v2_6_adaptive_hold_tau"
...
def load_adaptive_target_bias(path: str = ADAPTIVE_TARGET_BIAS_PATH) -> dict:
    ...
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if "rules" not in data or not isinstance(data["rules"], dict):
        raise RuntimeError(f"invalid adaptive target bias file: {path}")
    return data


def save_adaptive_target_bias(data: dict, path: str = ADAPTIVE_TARGET_BIAS_PATH) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
    data["type"] = "left_arm_v2_6_adaptive_target_bias"
    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2, sort_keys=True)
        f.write("\n")
...
def load_adaptive_hold_target_bias(path: str = ADAPTIVE_HOLD_TARGET_BIAS_PATH) -> dict:
    if not os.path.exists(path):
        return {"type": "left_arm_v2_6_adaptive_hold_target_bias", "rules": {}}
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if "rules" not in data or not isinstance(data["rules"], dict):
        raise RuntimeError(f"invalid adaptive hold target bias file: {path}")
    return data


def save_adaptive_hold_target_bias(data: dict, path: str = ADAPTIVE_HOLD_TARGET_BIAS_PATH) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
    data["type"] = "left_arm_v2_6_adaptive_hold_target_bias"
    data["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2, sort_keys=True)
        f.write("\n")
...
def adaptive_posture_key(low_shoulder_front: bool) -> Optional[str]:
    return LOW_SHOULDER_POSTURE if low_shoulder_front else None


def adaptive_target_bias_for(
    joint: str,
    ...
    if not record:
        return 0.0
    bias_deg = record.get("bias_deg") if isinstance(record, dict) else record
    if bias_deg is None:
        return 0.0
    return math.radians(max(-ADAPTIVE_TARGET_BIAS_LIMIT_DEG, min(ADAPTIVE_TARGET_BIAS_LIMIT_DEG, float(bias_deg))))


def adaptive_hold_target_bias_for(
    active: str,
    hold_targets: Dict[str, float],
    low_shoulder_front: bool = False,
    path: str = ADAPTIVE_HOLD_TARGET_BIAS_PATH,
) -> Dict[str, float]:
    posture = adaptive_posture_key(low_shoulder_front)
    if posture is None:
        return {}
    data = load_adaptive_hold_target_bias(path)
    active_rules = data.get("rules", {}).get(posture, {}).get(active, {})
    if not active_rules:
        return {}
    offsets = {}
    for hold_name, record in active_rules.items():
        if hold_name not in hold_targets:
            continue
        bias_deg = record.get("bias_deg") if isinstance(record, dict) else record
        if bias_deg is None:
            continue
        offsets[hold_name] = math.radians(
            max(-ADAPTIVE_HOLD_TARGET_BIAS_LIMIT_DEG, min(ADAPTIVE_HOLD_TARGET_BIAS_LIMIT_DEG, float(bias_deg)))
        )
    return offsets


def update_adaptive_hold_target_bias(
    active: str,
    induced_errors_deg: Dict[str, float],
    low_shoulder_front: bool = False,
    path: str = ADAPTIVE_HOLD_TARGET_BIAS_PATH,
    label: str = "",
) -> Dict[str, dict]:
    ...
    if posture is None:
        return {}
    data = load_adaptive_hold_target_bias(path)
    rules = data.setdefault("rules", {}).setdefault(posture, {}).setdefault(active, {})
    updates = {}
    for hold_name, error_deg in induced_errors_deg.items():
        ...
        if label:
            record["label"] = label
        rules[hold_name] = record
        updates[hold_name] = record
    if updates:
        save_adaptive_hold_target_bias(data, path)
    return updates


def update_adaptive_target_bias(
    final_errors_deg: Dict[str, float],
    low_shoulder_front: bool = False,
    path: str = ADAPTIVE_TARGET_BIAS_PATH,
    label: str = "",
) -> Dict[str, dict]:
    ...
    for joint, error_deg in final_errors_deg.items():
        ...
        best_bias = float(previous.get("best_bias_deg", current_bias)) if isinstance(previous, dict) else current_bias
        step_scale = float(previous.get("step_scale", 1.0)) if isinstance(previous, dict) else 1.0
        improved = abs(error_deg) < best_error - ADAPTIVE_HOLD_TAU_IMPROVEMENT_DEADBAND_DEG
        worse = (
            previous_error is not None
            and abs(error_deg) > previous_error + ADAPTIVE_HOLD_TAU_IMPROVEMENT_DEADBAND_DEG
        )
        if improved:
            best_error = abs(error_deg)
            best_bias = current_bias
        elif worse:
            current_bias = best_bias
            step_scale = max(ADAPTIVE_HOLD_TAU_MIN_STEP_SCALE, step_scale * 0.5)
        ...
        delta_bias = max(
            ...
            min(ADAPTIVE_TARGET_BIAS_MAX_STEP_DEG * step_scale, error_deg * ADAPTIVE_TARGET_BIAS_STEP_SCALE * step_scale),
        )
        new_bias = max(-ADAPTIVE_TARGET_BIAS_LIMIT_DEG, min(ADAPTIVE_TARGET_BIAS_LIMIT_DEG, current_bias + delta_bias))
        if new_bias == current_bias:
            continue
        record = {
            "bias_deg": new_bias,
    ...
    if updates:
        save_adaptive_target_bias(data, path)
    return updates


def adaptive_target_bias_ready_for(
    joint: str,
    error_deg: float,
    low_shoulder_front: bool = False,
) -> bool:
    if adaptive_hold_tau_converged_for(joint, low_shoulder_front):
        return True
    return abs(error_deg) < ADAPTIVE_HOLD_TAU_ERROR_DEADBAND_DEG


def adaptive_hold_tau_for(
    active: str,
    hold_targets: Dict[str, float],
    hold_tau: Dict[str, float],
    low_shoulder_front: bool = False,
    path: str = ADAPTIVE_HOLD_TAU_PATH,
) -> Dict[str, float]:
    posture = adaptive_posture_key(low_shoulder_front)
    if posture is None:
        return hold_tau
    data = load_adaptive_hold_tau(path)
    active_rules = data.get("rules", {}).get(posture, {}).get(active, {})
    if not active_rules:
        return hold_tau
    adapted = dict(hold_tau)
    for hold_name, record in active_rules.items():
        if hold_name not in hold_targets:
            continue
        ...
        if tau is None:
            continue
        adapted[hold_name] = clamp_tau(hold_name, float(tau), HOLD_TAU_LIMITS)
    return adapted
...
def update_adaptive_hold_tau(
    ...
    path: str = ADAPTIVE_HOLD_TAU_PATH,
    label: str = "",
    error_deadband_deg: float = ADAPTIVE_HOLD_TAU_ERROR_DEADBAND_DEG,
) -> Dict[str, dict]:
    posture = adaptive_posture_key(low_shoulder_front)
    if posture is None:
        return {}
    ...
    rules = data.setdefault("rules", {}).setdefault(posture, {}).setdefault(active, {})
    updates = {}
    for hold_name, error_deg in hold_errors_deg.items():
        if hold_name not in hold_tau or abs(error_deg) < error_deadband_deg:
            continue
        applied_tau = float(hold_tau[hold_name])
        ...
        delta_tau_mag = min(
            ...
            max(
                ADAPTIVE_HOLD_TAU_MIN_EXPLORATION_STEP,
                abs_error * ADAPTIVE_HOLD_TAU_STEP_PER_DEG * step_scale,
            ),
        )
        ...
        updates[hold_name] = record
    if updates:
        save_adaptive_hold_tau(data, path)
    return updates


def adaptive_hold_tau_converged_for(
    hold_name: str,
    low_shoulder_front: bool = False,
    path: str = ADAPTIVE_HOLD_TAU_PATH,
) -> bool:
    ...
    if posture is None:
        return False
    data = load_adaptive_hold_tau(path)
    posture_rules = data.get("rules", {}).get(posture, {})
    for active_rules in posture_rules.values():
        record = active_rules.get(hold_name) if isinstance(active_rules, dict) else None
        if adaptive_hold_tau_record_converged(record):
            return True
    return False


def adaptive_hold_tau_record_converged(record: object) -> bool:
...
def adaptive_hold_tau_converged_for_active(
    ...
    return adaptive_hold_tau_record_converged(record)


def direction_key(delta_deg: float) -> str:
    return "positive" if delta_deg >= 0 else "negative"


def nudge_hold_tau_for(
    active: str,
    hold_targets: Dict[str, float],
    low_shoulder_front: bool = False,
    adaptive_active: Optional[str] = None,
) -> Dict[str, float]:
    hold_tau = {
        hold_name: clamp_tau(hold_name, tau, HOLD_TAU_LIMITS)
        for hold_name, tau in HOLD_TAU_RULES["base"].items()
    }
    if low_shoulder_front:
        low_rules = HOLD_TAU_RULES[LOW_SHOULDER_POSTURE]
        for hold_name, tau in low_rules.get("*", {}).items():
            if hold_name in hold_targets:
                hold_tau[hold_name] = clamp_tau(hold_name, tau, HOLD_TAU_LIMITS)
        for hold_name, tau in low_rules.get(active, {}).items():
            if hold_name in hold_targets:
                hold_tau[hold_name] = clamp_tau(hold_name, tau, HOLD_TAU_LIMITS)
    return adaptive_hold_tau_for(adaptive_active or active, hold_targets, hold_tau, low_shoulder_front)


def nudge_active_tau_for(name: str, delta_deg: float, low_shoulder_front: bool = False) -> float:
...
def adaptive_hold_gains_for(
    ...
    if not low_shoulder_front:
        return hold_gains
    rules = POSTURE_HOLD_GAIN_RULES.get(LOW_SHOULDER_POSTURE, {})
    adapted = dict(hold_gains)
    for hold_name, gains in rules.get("*", {}).items():
        if hold_name in adapted:
            adapted[hold_name] = gains
    for hold_name, gains in rules.get(active, {}).items():
        if hold_name in adapted:
            adapted[hold_name] = gains
    return adapted
        ...
        if args.cmd == "nudge-hold":
            ...
            if target_limits:
                ...
                if target != requested_target:
                    print(
                        "v2.6 nudge-hold target clamped",
                        name,
                        "requested=",
                        requested_target,
                        "target=",
            ...
            for compliant_name in NUDGE_COMPLIANT_HOLDS_BY_ACTIVE.get(name, set()):
                if compliant_name in hold_gains:
                    hold_gains[compliant_name] = nudge_compliant_gains[compliant_name]
            hold_gains = adaptive_hold_gains_for(name, hold_gains, low_shoulder_front)
            ...
            nominal_hold_targets = dict(hold_targets)
            nudge_adaptive_active = f"{name}:{direction_key(args.deg)}"
            hold_tau = nudge_hold_tau_for(
                name,
                hold_targets,
                low_shoulder_front,
                adaptive_active=nudge_adaptive_active,
            )
            hold_target_offsets = {
                hold_name: offset
                for hold_name, offset in adaptive_hold_target_bias_for(
                    nudge_adaptive_active,
                    hold_targets,
                    low_shoulder_front,
                ).items()
            }
            if hold_target_offsets:
                ...
                print(
                    "v2.6 nudge-hold adaptive hold target bias=",
                    json.dumps({joint: math.degrees(offset) for joint, offset in hold_target_offsets.items()}, ensure_ascii=False),
                    "hold_targets=",
                    json.dumps({joint: hold_targets[joint] for joint in hold_target_offsets}, ensure_ascii=False),
                )
            active_tau = nudge_active_tau_for(name, args.deg, low_shoulder_front)
            if active_tau and name in NUDGE_ACTIVE_TAU:
                coupled_tau[name] = active_tau
            print(
                "v2.6 nudge-hold active",
                name,
                "delta_deg=",
                args.deg,
                "kp=",
            ...
            if hold_tau.get("shoulder_front") != NUDGE_HOLD_TAU.get("shoulder_front"):
                print(
                    "v2.6 nudge-hold adjusted shoulder_front hold_tau=",
            ...
            active_tau = nudge_active_tau_for(name, args.deg, low_shoulder_front)
            if active_tau:
                print("v2.6 nudge-hold active_tau=", {name: active_tau})
            if name in NUDGE_LOW_SHOULDER_GAINS and is_low_shoulder_front(current):
                print(
                    "v2.6 nudge-hold low shoulder gains active",
                    name,
                    "shoulder_front_pos=",
            ...
            low_shoulder_hold_overrides = {
                ...
                for hold_name in (
                    set(POSTURE_HOLD_GAIN_RULES[LOW_SHOULDER_POSTURE].get("*", {}))
                    | set(POSTURE_HOLD_GAIN_RULES[LOW_SHOULDER_POSTURE].get(name, {}))
                )
                if hold_name in hold_gains
            }
            if low_shoulder_hold_overrides and is_low_shoulder_front(current):
                print(
                    "v2.6 nudge-hold low shoulder adaptive hold gains=",
            ...
            if preload_seconds != 0.2:
                print("v2.6 nudge-hold preload_seconds=", preload_seconds)
            trajectory = NUDGE_TRAJECTORY.get(name, "smoothstep")
            linear_blend = NUDGE_LINEAR_BLEND.get(name, 0.0)
            if trajectory != "smoothstep":
                print("v2.6 nudge-hold trajectory=", trajectory, "linear_blend=", linear_blend)
            compliant_holds = {
            ...
            if compliant_holds:
                print("v2.6 nudge-hold compliant hold gains=", json.dumps(compliant_holds, ensure_ascii=False))
            step_deg = NUDGE_STEP_DEG.get(name, 0.0)
            if step_deg > 0:
                print("v2.6 nudge-hold step_deg=", step_deg)
            if len(coupled_targets) > 1:
                print(
                    "v2.6 nudge-hold coupled active targets=",
            ...
            if len(coupled_targets) > 1:
                ...
                return
            hold_baseline_errors = {
                hold_name: math.degrees(nominal_hold_targets[hold_name] - current[hold_name])
                for hold_name in nominal_hold_targets
                if hold_name in current
            }
            ...
            if name == "wrist_side" and is_low_shoulder_front(current):
                ...
                if abs(wrist_side_delta_deg) > NUDGE_LOW_SHOULDER_WRIST_SIDE_FINE_DEADBAND_DEG:
                    ...
                    print(
                        "v2.6 nudge-hold low shoulder wrist_side fine",
                        "delta_deg=",
                    ...
                    print(
                        "v2.6 nudge-hold low shoulder wrist_side fine skip delta_deg=",
                        wrist_side_delta_deg,
                        "deadband_deg=",
            ...
            induced_errors = {
                ...
                if abs(hold_baseline_errors.get(hold_name, 0.0)) < ADAPTIVE_TARGET_BIAS_ERROR_DEADBAND_DEG
                and abs(error) >= ADAPTIVE_TARGET_BIAS_ERROR_DEADBAND_DEG
            }
            if induced_errors:
                print(
                    "v2.6 nudge-hold adaptive hold tau observation active-induced errors",
                    json.dumps(
                        {
                            hold_name: {
                                "before_active": hold_baseline_errors.get(hold_name, 0.0),
                                "final": final_hold_errors[hold_name],
                            }
                            for hold_name in induced_errors
                        },
                        ensure_ascii=False,
                    ),
                )
                hold_tau_updates = update_adaptive_hold_tau(
                    nudge_adaptive_active,
                    induced_errors,
                    hold_tau,
                    low_shoulder_front=low_shoulder_front,
                    label="nudge-hold-induced",
                    error_deadband_deg=ADAPTIVE_TARGET_BIAS_ERROR_DEADBAND_DEG,
                )
                if hold_tau_updates:
                    print(
                        "v2.6 nudge-hold adaptive hold tau updates",
                        json.dumps(hold_tau_updates, ensure_ascii=False),
                    )
                offset_ready_errors = {
                    hold_name: error
                ...
                if not offset_ready_errors:
                    return
                hold_target_updates = update_adaptive_hold_target_bias(
                    nudge_adaptive_active,
                    offset_ready_errors,
                    low_shoulder_front=low_shoulder_front,
                    label="nudge-hold-induced",
                ...
                if hold_target_updates:
                    print(
                        "v2.6 nudge-hold adaptive hold target bias updates",
        ...
        if args.cmd == "home":
            ...
            for name in ordered:
                ...
                if name == "elbow" and "shoulder_front" in targets:
                    ...
                    print(
                        "v2.6 home coupled shoulder/roll/wrist active",
                        ",".join(group_names),
                        "hold joints:",
                        ", ".join(hold_targets.keys()),
                    )
                    for group_name in group_names:
                        ...
                        print("v2.6 home coupled shoulder/roll/wrist gains", group_name, "kp=", gains["kp"], "kd=", gains["kd"], "seconds=", coupled_seconds)
                    print("v2.6 home coupled shoulder/roll/wrist progress windows=", json.dumps(COUPLED_HOME_PROGRESS_WINDOWS, ensure_ascii=False))
                    if COUPLED_HOME_PRE_WINDOW_GAINS:
                        print("v2.6 home coupled shoulder/roll/wrist pre-window gains=", json.dumps(COUPLED_HOME_PRE_WINDOW_GAINS, ensure_ascii=False))
                    if command_bias:
                        print(
                            "v2.6 home coupled shoulder/roll/wrist command bias deg=",
                            json.dumps({name: COUPLED_HOME_COMMAND_BIAS_DEG[name] for name in command_bias}, ensure_ascii=False),
                        )
                    print("v2.6 home coupled shoulder/roll/wrist control_dt=", COUPLED_HOME_CONTROL_DT)
                    print("v2.6 home coupled shoulder/roll/wrist trajectory=", COUPLED_HOME_TRAJECTORY)
                    if COUPLED_HOME_TRAJECTORY == "blend_smootherstep":
                        print("v2.6 home coupled shoulder/roll/wrist linear_blend=", COUPLED_HOME_LINEAR_BLEND)
                    ...
                    print("v2.6 home coupled shoulder/roll/wrist reached=", json.dumps(reached, ensure_ascii=False))
        ...
        if args.cmd == "clearance":
            ...
            for name in ordered:
                ...
                if (
                    ...
                    print(
                        "v2.6 clearance coupled shoulder/roll/wrist active",
                        ",".join(group_names),
                        "hold joints:",
                        ", ".join(hold_targets.keys()),
                    )
                    for group_name in group_names:
                        gains = move_gains[group_name]
                        print("v2.6 clearance coupled shoulder/roll/wrist gains", group_name, "kp=", gains["kp"], "kd=", gains["kd"], "seconds=", coupled_seconds)
                    print(
                        "v2.6 clearance coupled shoulder/roll/wrist continuous interpolation steps_removed=",
                        coupled_steps,
                        "raw_seconds=",
                        raw_coupled_seconds,
                        "used_seconds=",
                    ...
                    print("v2.6 clearance coupled shoulder/roll/wrist progress windows=", json.dumps(COUPLED_CLEARANCE_PROGRESS_WINDOWS, ensure_ascii=False))
                    print("v2.6 clearance coupled shoulder/roll/wrist pre-window gains=", json.dumps(COUPLED_CLEARANCE_PRE_WINDOW_GAINS, ensure_ascii=False))
                    print("v2.6 clearance coupled shoulder/roll/wrist velocity_ff_joints=", ",".join(sorted(COUPLED_CLEARANCE_VELOCITY_FF_JOINTS)))
                    print("v2.6 clearance coupled shoulder/roll/wrist move_tau_ff=", json.dumps(COUPLED_CLEARANCE_MOVE_TAU_FF, ensure_ascii=False))
                    print("v2.6 clearance coupled shoulder/roll/wrist control_dt=", COUPLED_CLEARANCE_CONTROL_DT)
                    print("v2.6 clearance coupled shoulder/roll/wrist trajectory=", COUPLED_CLEARANCE_TRAJECTORY)
                    if COUPLED_CLEARANCE_TRAJECTORY == "blend_smootherstep":
                        print("v2.6 clearance coupled shoulder/roll/wrist linear_blend=", COUPLED_CLEARANCE_LINEAR_BLEND)
                    arm.enable(group_names + list(hold_targets.keys()))
                    arm.move_targets_with_holds(
                        group_targets,
                        seconds_per_step=coupled_seconds,
                    ...
                    reached = arm.positions(group_names)
                    print("v2.6 clearance coupled shoulder/roll/wrist reached=", json.dumps(reached, ensure_ascii=False))
                    if "wrist" in group_names:
                        ...
                        if abs(wrist_delta_deg) > CLEARANCE_WRIST_FINE_DEADBAND_DEG:
                            ...
                            print("v2.6 clearance coupled wrist fine hold gains=", json.dumps(fine_hold_gains, ensure_ascii=False))
                            if CLEARANCE_WRIST_FINE_SETTLE_SECONDS > 0:
                                ...
                                print("v2.6 clearance coupled wrist settle before fine seconds=", CLEARANCE_WRIST_FINE_SETTLE_SECONDS)
                            ...
                            print(
                                "v2.6 clearance coupled wrist fine target delta_deg=",
                                wrist_delta_deg,
                                "bias_deg=",
                                fine_bias_deg,
                                "target=",
                            ...
                            print(
                                "v2.6 clearance coupled wrist fine skip delta_deg=",
                                wrist_delta_deg,
                                "deadband_deg=",
                ...
                if name == "wrist":
                    if CLEARANCE_WRIST_FINE_SETTLE_SECONDS > 0:
                        ...
                        print("v2.6 clearance wrist settle before fine seconds=", CLEARANCE_WRIST_FINE_SETTLE_SECONDS)
                        arm.hold_positions_with_gains(
                            settle_targets,
                            seconds=CLEARANCE_WRIST_FINE_SETTLE_SECONDS,
                            gains=settle_gains,
                    ...
                    if abs(wrist_delta_deg) > CLEARANCE_WRIST_FINE_DEADBAND_DEG:
                        ...
                        print(
                            "v2.6 clearance wrist fine target delta_deg=",
                            wrist_delta_deg,
                            "bias_deg=",
                            fine_bias_deg,
                            "target=",
                        ...
                        print(
                            "v2.6 clearance wrist fine skip delta_deg=",
                            wrist_delta_deg,
                            "deadband_deg=",
                            CLEARANCE_WRIST_FINE_DEADBAND_DEG,
                        )
