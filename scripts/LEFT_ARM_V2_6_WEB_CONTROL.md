...
```bash
python3 left_arm_v2_6_web_control.py --enable-execute --sudo
```
...
If a stable final residual of at least 1.0 degree remains after hold-tau learning reaches plateau/backoff with enough samples, replay learns a feed-forward target bias in `scripts/data/left_arm_v2_6_adaptive_target_bias.json`; the bias is applied next time when that joint reaches its own active replay step.
Residuals that are above the target-bias threshold but below the hold-tau learning threshold are treated as small position residuals and may enter target-bias learning without waiting for hold-tau convergence.
